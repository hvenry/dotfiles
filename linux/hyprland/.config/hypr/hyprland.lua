...
hl.config({
	...
	input = {
		...
		touchpad = {
			natural_scroll = true,
			-- Require a real physical click; tapping the surface does nothing.
			tap_to_click = false,
...
hl.bind(
	mod .. " + V",
	hl.dsp.exec_cmd(
		[[bash -c 'selected=$(cliphist list | rofi -dmenu -p "Clipboard") && echo "$selected" | cliphist decode | wl-copy']]
	)
)
